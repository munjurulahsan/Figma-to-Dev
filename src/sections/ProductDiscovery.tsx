...
export function ProductDiscovery() {
  ...
  return (
    ...
    >
      <RevealOnScroll className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <AnimatePresence mode="popLayout">
          {filteredProducts.map((product, i) => (
            <motion.article
              ...
              className="group flex flex-col bg-surface-deep p-4 transition-shadow duration-500 ease-out hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.8)]"
            >
              ...
              <div className="mb-1 flex items-start justify-between gap-2">
                <h3 className="font-body text-[15px] font-semibold uppercase text-ink-100 transition-colors duration-200 group-hover:text-ink-300">
                  {product.name}
                </h3>
                <span className="font-mono text-xs font-medium tracking-[0.24px] text-ink-300">
                  {product.price}
                </span>
              </div>
              <p className="mb-4 font-mono text-[11px] uppercase text-ink-200">{product.meta}</p>

              <button className="bg-surface-3 py-3 font-mono text-[11px] font-medium uppercase tracking-[0.55px] text-ink-100 transition-all duration-200 hover:bg-ink-100 hover:text-surface-0 hover:shadow-md">
                Add to Bag +
              </button>
            </motion.article>
          ))}
